...
suppressPackageStartupMessages({
  required_packages <- c(
    ...
    "corrplot", "reshape2", "caret", "pROC", "keras3"
  )
})
...
# Force TensorFlow to be available since we installed it
tf_ok <- TRUE

## Paths ------------------------------------------------------------------------------
# Use current working directory as root when executed via Rscript
root_dir <- normalizePath(getwd())
data_path <- file.path(root_dir, "CreditCard-Dataset", "CreditCard.csv")
...
## Quick info ------------------------------------------------------------------------
message(sprintf("Rows: %s, Cols: %s", nrow(df), ncol(df)))

## Plot helper -----------------------------------------------------------------------
save_plot <- function(p, filename, width = 7, height = 5) {
  ggplot2::ggsave(
    filename = file.path(images_dir, filename), plot = p + theme_bw(),
...
p_limit <- ggplot(df, aes(x = LIMIT_BAL)) +
  ...
  labs(title = "Fig.1: Credit Limit", x = "Credit Limit (NT dollar)", y = "Count")
save_plot(p_limit, "fig01_limit_bal.png", width = 7, height = 5)

# LIMIT_BAL default overlay
...
save_plot(p_limit_overlay, "fig01b_limit_bal_overlay.png", width = 7, height = 5)

# Gender bar
p_sex <- df %>%
  mutate(SEX_F = factor(SEX, levels = c(1, 2), labels = c("Male", "Female"))) %>%
  count(SEX_F, !!sym(target_col)) %>%
  group_by(SEX_F, `default` = !!sym(target_col)) %>%
  summarise(n = sum(n), .groups = "drop") %>%
  ggplot(aes(x = SEX_F, y = n, fill = factor(default))) +
  geom_col(alpha = 0.7) +
  scale_fill_manual(values = c("0" = "#cc79a7", "1" = "#0072b2"), name = "Default") +
...
save_plot(p_edu, "fig03_education.png", width = 7.5)
...
## Payment status PAY_0 .. PAY_6 (named PAY_0, PAY_2, ..., PAY_6) -------------------
pay_cols <- c("PAY_0", "PAY_2", "PAY_3", "PAY_4", "PAY_5", "PAY_6")
stopifnot(all(pay_cols %in% names(df)))

plot_delay <- function(cn) {
  # build frequency for bins -2..9
  vals <- -2:9
  df %>%
    mutate(val = .data[[cn]]) %>%
...
pay_plots <- lapply(pay_cols, plot_delay)
save_plot(plot_grid(plotlist = pay_plots, ncol = 2), "fig06_payment_status.png", width = 12, height = 10)

## BILL_AMT1..6 histograms (log y) ---------------------------------------------------
bill_cols <- paste0("BILL_AMT", 1:6)
stopifnot(all(bill_cols %in% names(df)))
...
save_plot(plot_grid(plotlist = lapply(bill_cols, plot_bill), ncol = 2), "fig07_bill_amount.png", width = 10, height = 10)

## PAY_AMT1..6 histograms (log y) ----------------------------------------------------
pay_amt_cols <- paste0("PAY_AMT", 1:6)
stopifnot(all(pay_amt_cols %in% names(df)))
...
save_plot(plot_grid(plotlist = lapply(pay_amt_cols, plot_pay_amt), ncol = 2), "fig08_prev_payment.png", width = 10, height = 10)

## 2) Correlation heatmap -------------------------------------------------------------
num_df <- df %>% dplyr::select(-ID)
cor_mat <- suppressWarnings(cor(num_df, use = "pairwise.complete.obs"))
png(file.path(images_dir, "fig09_correlation_heatmap.png"), width = 1800, height = 1800, res = 200, bg = "white")
corrplot(cor_mat, method = "color", type = "full", tl.cex = 0.6, tl.col = "black", order = "original")
dev.off()
...
ratio <- mean(y == 1)
message(sprintf("Default ratio: %.4f", ratio))
...
if (tf_ok) {
  ...
  p1 <- ggplot(dfh, aes(epoch)) +
    ...
    labs(title = "Training vs Validation Loss", y = "Loss", x = "Epoch", color = NULL) + theme_bw()
  p2 <- ggplot(dfh, aes(epoch)) +
    ...
    labs(title = "Training vs Validation Accuracy", y = "Accuracy", x = "Epoch", color = NULL) + theme_bw()
  save_plot(plot_grid(p1, p2, ncol = 1), "fig10_training_history.png", width = 8, height = 8)
} else {
  message("TensorFlow not available — skipping training. Generating placeholder charts.")
  placeholder <- ggplot() +
    annotate("text", x = 0, y = 0, label = "TensorFlow not available. Training skipped.") +
...
if (tf_ok) {
  ...
  metrics_txt <- file.path(images_dir, "metrics.txt")
  cat(
  ...
  writeLines("TensorFlow not available. Training and evaluation skipped.", file.path(images_dir, "metrics.txt"))
  placeholder <- ggplot() +
    annotate("text", x = 0, y = 0, label = "TensorFlow not available. ROC unavailable.") +
    xlim(-1, 1) + ylim(-1, 1) + theme_bw()
  save_plot(placeholder, "fig11_roc.png", width = 6, height = 5)
}

message("Done. Images saved to: ", images_dir)
