...
build_image() {
    ...
    if docker build -t tensorflow-mnist-app .; then
...
run_container() {
    ...
    if docker run --rm --memory=4g tensorflow-mnist-app; then
...
# Main function
main() {
    echo "=== Dockerized TensorFlow MNIST Application Build and Run Script ==="
    echo
