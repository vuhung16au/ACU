 ...
 * Builds a sorted array one element at a time by repeatedly taking the next element 
 * and inserting it into its correct position in the sorted part of the array.
 */
void insertionSort(int arr[], int n) {
    int i, key, j;
    for (i = 1; i < n; i++) {
        key = arr[i]; // Element to be inserted into sorted array
        j = i - 1;
...
int main(int argc, char* argv[]) {
    printf("C Insertion Sort Performance Test\n");
    printf("==================================\n");
    ...
    insertionSort(data_copy, data_size);
    ...
    fprintf(result_file, "C Insertion Sort Results\n");
