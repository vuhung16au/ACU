#!/usr/bin/env python3
"""
Radix Sort implementation in Python with performance measurement.
"""
import time
import sys
import os
...
def counting_sort_for_radix(arr, exp):
    ...
    Performs counting sort based on the digit at position 'exp'.
    """
    n = len(arr)
    output = [0] * n
...
def radix_sort(arr):
    ...
    """
    # Handle negative numbers by finding the minimum value
    min_val = min(arr)
    if min_val < 0:
        # Shift all values to positive if there are negative values
        temp_arr = [x - min_val for x in arr]
    else:
        temp_arr = arr.copy()
    ...
    exp = 1
    while max_val // exp > 0:
        counting_sort_for_radix(temp_arr, exp)
        exp *= 10
    
    # If we had negative numbers, we need to shift back
    if min_val < 0:
        for i in range(len(arr)):
            arr[i] = temp_arr[i] + min_val
    else:
        for i in range(len(arr)):
            arr[i] = temp_arr[i]
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Python Radix Sort Performance Test')
...
def main():
    print("Python Radix Sort Performance Test")
    print("=" * 40)
    ...
    with open(result_filename, "w") as f:
        f.write(f"Python Radix Sort Results\n")
